...
export default function Services({ data }: { data: Service[] }) {
  ...
  if (isMobileRef.current) {
    return (
      <section id="services" className="px-4 py-20">
        <p className="font-mono text-xs text-secondary/70 tracking-widest uppercase text-center mb-2">// Selected Work</p>
        <h2 className="font-display italic text-4xl text-foreground text-center mb-12">Services</h2>
        <div className="flex flex-col gap-5">
          {data.map((s, i) => (
            <MobileServiceCard key={i} item={s} index={i} onClick={() => setSelected(s)} />
          ))}
        </div>
        {selected && (
          <DetailPage item={{ ...selected, type: 'service' }} onClose={() => setSelected(null)} />
        )}
      </section>
    );
  }
  ...
  return (
    <>
      <section id="services" ref={sectionRef} style={{ height: stickyHeight }} className="relative">
        <div className="sticky top-0 h-screen flex flex-col items-center justify-center overflow-hidden">
          <p className="font-mono text-xs text-secondary/70 tracking-widest uppercase mb-2">// Selected Work</p>
          <h2 className="font-display italic text-5xl md:text-6xl text-foreground mb-6">Services</h2>
              ...
              >
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="font-mono text-xs text-muted-foreground">{service.year}</span>
                  </div>
                  <h3 className="font-display text-xl text-foreground mb-1.5">{service.title}</h3>
                  <p className="font-body text-xs text-muted-foreground leading-relaxed mb-3 line-clamp-2">{service.description}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {service.tags.map(tag => (
                      <span key={tag} className="px-2 py-0.5 rounded-full text-[10px] font-mono"
                        style={{ background: 'hsla(192,100%,50%,0.08)', border: '1px solid hsla(192,100%,50%,0.18)', color: 'hsl(192,100%,65%)' }}>
                        {tag}
                      </span>
                    ))}
                  </div>
...
function MobileServiceCard({ item, index, onClick }: { item: Service; index: number; onClick: () => void }) {
  return (
    <button onClick={onClick} className="w-full text-left rounded-2xl overflow-hidden transition-transform active:scale-[0.98]"
      ...
      <div className="relative h-40">
        ...
        <span className="absolute top-2 right-2 flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-mono"
